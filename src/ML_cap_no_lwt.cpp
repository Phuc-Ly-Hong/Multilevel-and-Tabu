...
struct Solution {
    ...
    vector<double> route_capacity_violation; // vi phạm tải trọng theo từng xe
...
double alpha2 = 1.0; // penalty cho capacity
...
void read_dataset(const string &filename){
    ...
    while (getline(file,line)){
        ...
        static int id = 1;
        ss >> x >> y >> demand;
        nodes.push_back({id++,x,y,1.0,demand});
    }
    file.close();
